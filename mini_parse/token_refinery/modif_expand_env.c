...
/*   Updated: 2026/05/10 09:30:37 by thantoni         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */

#include "mini_parse.h"

static size_t	_insert_var(char *holder, char *raw, t_env *m_env_list, int in_double)
{
	t_env	*found;
	size_t	i;
	...
	if (found != NULL)
	{
		while (found->m_val[i])
		{
			if (!in_double && (found->m_val[i] == ' ' || found->m_val[i] == '\t'))
				holder[i] = '\x1F';
			else
